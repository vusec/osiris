...
do
	APP_DIR=`hget $a`
	echo Memory experiment for app ${a}...
	CONFS="app"
	if [ "$a" == "spec" ]; then
...
#
# Merge Memory experiment results
#
RESULT_FILES=`find results -name memory.overhead.ini | sort`
if [ "$RESULT_FILES" != "" ]; then
	rm -f results/memory.overhead.all.ini
	for f in $RESULT_FILES
	do
		cat $f >> results/memory.overhead.all.ini
	done
fi
