...
static inline void util_cmdsvr_from_env(util_cmdsvr_conf_t *conf) {
    ...
    /* Set defaults. */
    if (!conf->dir) {
        conf->dir = _UTIL_CMDSVR_DEFAULT_DIR;
    }
    if (!conf->file) {
        conf->file = _UTIL_CMDSVR_DEFAULT_FILE;
    }
...
static void *util_cmdsvr_func(void *conf_arg)
{
    ...
    req->buff = _UTIL_MALLOC(req->max_size+1);
    ...
    do {
        ...
        if ((req->size = recv(conf->conn_fd, req->buff,
                req->max_size, 0)) < 0) {
        ...
        if (util_cmdsvr_parse_req(req) < 0) {
            _UTIL_PRINTF("cmdsvr: Received bad request (cmd=%d)\n", req->cmd);
            goto loop_end;
        }
        cb_ret = conf->cb(conf);
